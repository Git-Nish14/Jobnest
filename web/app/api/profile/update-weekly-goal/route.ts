...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = await checkRateLimit(`update-weekly-goal:${user.id}`, {
      maxRequests: 20,
      windowMs: 60_000,
    });
    ...
    const { error } = await supabase.auth.updateUser({
      data: { weekly_goal: body.weeklyGoal },
    });

    if (error) {
      console.error("Failed to update weekly goal:", error);
      throw ApiError.internal("Failed to update weekly goal");
    }

    return successResponse({ weeklyGoal: body.weeklyGoal });
  } catch (error) {
