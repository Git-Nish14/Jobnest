...
export interface CompletenessExtras {
  hasResumeDoc?: boolean;
  hasCoverLetterDoc?: boolean;
}
...
export function computeCompleteness(app: JobApplication, extras?: CompletenessExtras): CompletenessResult {
  const results = CHECKS.map((c) => {
    let met = c.met(app);
    if (c.label === "Resume uploaded")  met = met || !!extras?.hasResumeDoc;
    if (c.label === "Cover letter")     met = met || !!extras?.hasCoverLetterDoc;
