...
The word JOBNEST confirms that the user already applied. Do not ask whether they applied. Set status to Applied and use today's date for applied_date unless the conversation explicitly states a different application date. Do not ask for the date. Ask only if company, job title, or job_url cannot be recovered from the conversation.
...
Build the tool arguments from this sample. Replace sample values with facts from the conversation, keep job_url and job_description every time, and omit optional keys whose values are unknown. Never send placeholders, empty strings, or null for optional fields:
{
...
Field limits: request_id 100 characters; company 255; position 255; job_id 100; job_url required, 2083, and HTTP(S) only; salary_range 100; location 255; notes 5000; job_description required and 20000. applied_date must be YYYY-MM-DD. status must be Applied, Phone Screen, Interview, Offer, Rejected, Withdrawn, or Ghosted. source and ats_provider must use their schema enum values. requires_sponsorship is boolean. company_tier must be FAANG, Tier 1, Tier 2, Tier 3, or Startup. glassdoor_rating must be 1.0-5.0 with at most one decimal.
