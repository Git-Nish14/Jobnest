...
export const chatGptApplicationSchema = z.object({
  ...
  job_description: z.string().trim().max(20000).optional().describe("The complete job-description text available anywhere in the conversation, up to 20,000 characters. Include it even if it appeared earlier and was already used for resume tailoring. Preserve all known responsibilities, qualifications, skills, experience, education, benefits, employment type, schedule, and employer details instead of summarizing them. Never invent missing text."),
