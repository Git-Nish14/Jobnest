...
// Warm Atelier palette — maps each stage of the job search funnel
  ...
  if (total === 0 || active.length === 0) {
    return (
      <div className="db-panel h-full flex flex-col">
        ...
        <p className="text-xs text-[#55433d] mb-6">Track your progress through each stage</p>
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
          <p className="text-[#55433d] text-sm">No applications yet.</p>
  ...
  return (
    <div className="db-panel h-full flex flex-col gap-5">
      <div>
        ...
        <p className="text-xs text-[#55433d] mt-0.5">Your progress through each hiring stage</p>
      </div>
      ...
      <div className="flex items-center gap-5">
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-14 h-14 rounded-full bg-white flex flex-col items-center justify-center">
              <span className="text-base font-bold text-[#1a1c1b] leading-none">{total}</span>
              <span className="text-[9px] text-[#55433d] uppercase tracking-wide font-semibold">total</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-2.5 min-w-0">
          <div className="flex justify-between items-center">
            <span className="text-xs text-[#55433d]">Response rate</span>
            <span className="text-xs font-bold text-[#99462a]">{responseRate}%</span>
          </div>
          <div className="h-1.5 w-full rounded-full bg-[#f4f3f1] overflow-hidden">
            <div className="h-full rounded-full bg-[#99462a]" style={{ width: `${responseRate}%` }} />
          </div>
          <div className="flex justify-between items-baseline pt-1">
            <div className="text-center">
              <p className="text-base font-bold text-[#1a1c1b]">{inProgress}</p>
              <p className="text-[10px] text-[#55433d]">In progress</p>
            </div>
            <div className="text-center">
              <p className="text-base font-bold text-[#006d34]">{offers}</p>
              <p className="text-[10px] text-[#55433d]">Offer{offers !== 1 ? "s" : ""}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2 flex-1">
        {segments.map((s) => (
          <div key={s.status} className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#55433d] flex-1 truncate">{s.label}</span>
            <span className="text-xs font-semibold text-[#1a1c1b] shrink-0 tabular-nums">{s.count}</span>
            <span className="text-[10px] text-[#55433d] w-8 text-right shrink-0">{s.pct.toFixed(0)}%</span>
          </div>
        ))}
      </div>
    </div>
