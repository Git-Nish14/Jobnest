...
- [ ] **URL field — no size restriction, fully validated** — `job_url` (and any other URL field: `demo_url`, `repo_url`, `credential_url`, `meeting_url`, shared-link tokens, import-url endpoint) must accept URLs of arbitrary length with no hard truncation; validate with a strict allowlist approach: scheme must be `https://` (or `http://` for localhost/dev only), block private/loopback ranges via `assertSafeUrl()` (already used on parse-jd — extend to all URL inputs), reject `javascript:` / `data:` / `vbscript:` schemes, strip null bytes and path-traversal segments, enforce max 2 083 chars only as a DB column safety floor (not a UX restriction); surface a clear inline validation message if the URL is malformed, not a generic error
- [ ] **Timestamps — device-local timezone throughout** — every date/time shown to the user (`applied_date`, `remind_at`, `scheduled_at`, `created_at`, activity timeline, streak counter, digest email send times, OPT countdown, interview countdowns, document upload dates) must render in the **user's local timezone** (detected via `Intl.DateTimeFormat().resolvedOptions().timeZone` on the client, or stored as a user preference in `user_metadata.timezone`); avoid mixing UTC display with local display on the same page; use a single shared `formatLocalDateTime(date, tz)` utility (e.g. wrapping `date-fns-tz` or `Intl.DateTimeFormat`) so all pages stay consistent; server-rendered timestamps must either be rendered client-side or pass the resolved timezone as a prop
- [ ] **Disable browser autocomplete / cached suggestions on all form fields** — set `autocomplete="off"` on every sensitive or app-specific input across the site (job URL, company name, position, salary, OTP, password fields, search bar, NESTAi chat input, document label, contact fields, template editor, etc.); for password fields use `autocomplete="new-password"` (current) and `autocomplete="current-password"` (login) so the browser password manager still works but generic form-fill cache suggestions do not appear; audit every `<input>`, `<textarea>`, and React Hook Form field to ensure no field shows browser-cached dropdown ghosts; also suppress browser-native datalist suggestions (`list` attribute) unless intentionally added
- [ ] **Application source — brand colours per platform** — the `source` badge on application cards and the source filter pills should use the official brand colour of each job board: LinkedIn `#0A66C2` (blue), Indeed `#003A9B` (navy), Glassdoor `#0CAA41` (green), Wellfound (AngelList) `#000000` (black), Dice `#EB1C26` (red), Handshake `#E8552A` (orange), Referral `#7C3AED` (violet — implies human trust), Recruiter Outreach `#F59E0B` (amber), Job Fair `#0891B2` (cyan), Company Website `#6B7280` (neutral grey), Other `#9CA3AF` (light grey); apply as a coloured left-accent bar or a tinted pill badge matching the existing `db-status-badge` pattern; dark mode variants required
...
- [x] **GitHub integration** — OAuth scope `read:user public_repo`; PKCE-style HMAC-signed state cookie; callback upserts `github_connections` (token + profile stats) and `github_repos` (up to 100 repos); profile card on profile page (avatar, bio, stats, repos); pin up to 6 repos for portfolio display; manual sync endpoint (5/hr rate limit); daily cron `github-sync` at 04:00 UTC syncs all connected users; disconnect purges connection + cached repos; `GITHUB_CLIENT_ID` + `GITHUB_CLIENT_SECRET` required
...
- [x] **Vitest tests — 983 tests, 70 files, 100% pass (no browser, fully automated)**
