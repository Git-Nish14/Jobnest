...
- [ ] **Auto-purge documents on Rejected applications (30-day grace, user-confirmed)** — when an application's status is set to `Rejected`, start a 30-day countdown to delete all Storage files attached to it via `application_documents` (PDFs, DOCs, etc.); the application row and all its DB data are kept — only the physical files in Supabase Storage are deleted to free quota; notify the user in-app (notification bell) and by email every 5 days from day 1 until they take action; each notification must include three explicit choices: **(1) Confirm delete** — user is happy for files to be purged on day 30 as scheduled; **(2) Keep files** — user overrides the auto-purge for this application permanently (sets a `retain_documents` flag, no further reminders); **(3) Save documents to library** — copies the files to `is_master = true` in the document library so they survive independently of the application before the purge runs; implementation outline: add `rejected_at TIMESTAMPTZ` and `retain_documents BOOLEAN NOT NULL DEFAULT FALSE` columns to `job_applications` (new migration); set `rejected_at = NOW()` in the status-change path whenever status transitions to `Rejected` (clear it if status later changes away from Rejected); add a daily cron route `POST /api/cron/purge-rejected-docs` (alongside existing `orphan-cleanup` and `process-deletions`) that: queries applications where `status = 'Rejected' AND rejected_at <= NOW() - INTERVAL '30 days' AND retain_documents = FALSE`, deletes their Storage objects and `application_documents` rows (keep RLS + ownership check), logs to `activity_logs`; add a separate daily cron (or extend the `overdue-reminders` cron) to send day-1, day-5, day-10, day-15, day-20, day-25 notifications for each affected application that hasn't been actioned; add `POST /api/applications/[id]/retain-documents` (sets `retain_documents = TRUE`, stops all further reminders for that application); "Save to library" action calls the existing document copy flow then sets `retain_documents = TRUE`; rate-limit all three user-action endpoints; add E2E tests for the cron, the notification cadence, and each of the three user choices
