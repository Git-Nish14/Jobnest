...
- [x] **`DELETE /api/applications/:id` API route** — proper server-side delete replacing the bare browser Supabase client call; includes CSRF origin check, authenticated `getUser()`, ownership verification, rate limiting (30/min), Storage cleanup of **all** document version paths + legacy `resume_path`/`cover_letter_path` before the DB delete, and ownership path-prefix guard (`startsWith(user.id + "/")`) so no cross-user files are ever removed; DB delete cascades to `application_documents`, `document_purge_queue`, `activity_logs`, etc.
- [x] **`DeleteApplicationButton` component** — client component added to the application detail page header (next to Edit); shows "Delete" → inline "Are you sure? · Yes, delete · Cancel" strip; confirming calls the DELETE API and navigates to `/applications`; auto-cancels after 5 seconds if no confirm
- [x] **Tests** — 1265 tests / 83 files / 100% pass; `delete-application.test.ts` (11 unit tests: auth gates, ownership check, full happy path, all-version Storage path collection, legacy path inclusion, cross-user path filtering, non-fatal storage error, DB error → 500); **Playwright E2E** (`tests/e2e/application-delete.spec.ts`, 7 authenticated tests with real Supabase backend): unauthenticated 401 guard, card dropdown menu stays open after first click + confirm removes card + cancel leaves app intact, detail page button visible + confirm strip UI + cancel stays on page + confirm navigates away with app gone; all tests self-cleaning (create then delete real applications); lint ✓ · TSC ✓ · build ✓ · coverage above thresholds

---

## ✅ Sprint Shipped (May 2026 — Bug fixes, UX polish, document auto-purge & security hardening)

- [x] **PDF upload — preserve original filename** — application form uploads now insert an `application_documents` row with `original_name = file.name`; `mime_type` hardcoded to `"application/pdf"` (no longer trusts browser-supplied `File.type`); `user_id` defence-in-depth guard added to the post-upload file-path UPDATE; DocumentManager now shows the real filename (e.g. `John_Doe_Resume.pdf`) instead of the generic path segment `"resume"` / `"cover_letter"`
- [x] **TC / Salary comparison fixed** — "Offer Comparison" table renamed to "Salary Comparison" and now shows **all** applications with salary data (not just `status = "Offer"`); a `StatusBadge` column per row provides at-a-glance context; redundant "All Salary Information" secondary list removed; the TC legend and Export PDF button guard updated to `allWithSalary.length`
- [x] **Rejected status dot & colour fixed** — every status in `STATUS_META` now carries its own `ring` token (e.g. `ring-red-500/25` for Rejected, `ring-amber-400/25` for Applied) so the current-stage ring uses the status' own colour instead of always terracotta; Rejected dot gains `dark:bg-red-400` so it's visible in dark mode; mobile dot unified to `w-3 h-3` (was non-standard `w-4.5 h-4.5` with `flex items-center justify-center` which rendered as a ring artefact)
- [x] **Duplicate application inline warning** — debounced (600 ms) `useEffect` watches the `company` + `position` fields; queries the user's own applications via Supabase (RLS-protected) for a case-insensitive exact match; shows a non-blocking amber inline note below the Position field; excludes the current application when editing; Supabase builder immutability bug fixed (`.neq()` result now captured)
- [x] **Dashboard chart colours** — Monthly Breakdown: Applied → `amber-500`, Rejected → `red-500` (was grey — semantically wrong), Offers stay `emerald-500`; Stage Funnel: warm-to-cool gradient (amber → orange → terracotta → emerald-600 → emerald-700) instead of uniform terracotta; all 3 series have distinct fills
- [x] **Auto-purge documents on Rejected applications** — DB trigger (`schedule_document_purge`) fires on `status → Rejected` and inserts into `document_purge_queue` with `purge_at = NOW() + 30 days`; daily cron `GET /api/cron/purge-rejected-documents` (03:00 UTC) deletes **all** Storage file versions (not just `is_current=true`), marks document rows non-current, marks queue entry "purged", and sends a "files deleted" in-app notification (only when files actually existed); sends countdown notifications every 5 days during the grace period; `POST /api/applications/[id]/retain-documents` cancels the purge (`action=retain`) or copies current docs to the master library first then cancels (`action=library`); `DocumentPurgeBanner` on the application detail page shows a countdown with "Save to library" and "Keep files" actions; `document_purge_queue` RLS explicitly scoped `TO authenticated` with `REVOKE ALL … FROM anon`; `fetchError.message` no longer leaked in cron HTTP response; migration 35
- [x] **Security fixes** — storage path ownership guard (`startsWith(user.id + "/")`) added before `admin.storage.copy()` in `retain-documents`; `REVOKE ALL ON document_purge_queue FROM anon` added to migration; raw Supabase error strings replaced with generic messages in cron HTTP responses
- [x] **Tests** — 1254 tests / 82 files / 100% pass; 3 new test files: `purge-rejected-documents.test.ts` (24 tests: auth, purge branch with all-versions path fix, path guard, no-file edge case, storage error handling, notification branch with 5-day interval, mixed queue), `retain-documents.test.ts` (13 tests: auth/validation, retain/library actions, storage-path ownership guard, copy-failure resilience, destination-path sanitisation), `salary-status-badge.test.ts` (17 tests: status→colour token map, monthly-trends semantic colours, stage-funnel warm→cool gradient); lint ✓ · TSC ✓ · build ✓ · coverage above thresholds

---

## ✅ Sprint Shipped (May 2026 — JSON Autofill, Security & Tests)

- [x] **"Fill from JSON" — AI-assisted application autofill** — new "Fill from JSON" button on `/applications/new` (alongside "Import from job posting" and "Fill from resume"); two-tab modal: **"Get AI prompt"** (copyable structured prompt with today's date, field reference, and enum options for all 13 fields — source, ATS provider, company tier) and **"Paste JSON"** (paste AI response → import); client-side parse only, no API key needed; works with ChatGPT, Claude, Gemini, or any AI; `lib/utils/resume-json.ts`: `getApplicationImportPrompt()` + `parseApplicationJSON()` with null-byte stripping, real calendar-date validation (`Date.parse` after regex), URL normalisation to canonical `href`, dangerous scheme rejection, enum case-insensitive matching, `requires_sponsorship` boolean coercion (boolean | "true"/"false" | 0/1), markdown fence auto-strip; `ApplicationJsonImport` modal: `useMemo` for prompt (stable across renders), three-state copy button (idle/copied/failed — no deprecated `execCommand` DOM injection), warning panel stays open on partial imports so skipped fields are visible; security fixes shipped with this feature: null-byte injection in all string fields, impossible calendar dates (e.g. "2026-99-99") rejected, raw AI strings normalised before storage; **59 new unit tests** (`tests/unit/lib/utils/resume-json.test.ts`): happy path, fence stripping, error cases, security (null-byte, URL scheme, date), enum coercion, length caps; **Playwright E2E spec** (`tests/e2e/json-import.spec.ts`): unauthenticated redirect guard + 8 authenticated tests (button presence, modal open/close, prompt tab, happy-path import, warning path, error path, fence stripping, disabled state) — skipped automatically without `E2E_TEST_EMAIL`/`E2E_TEST_PASSWORD`; `resume-json.ts` hits 100% statements / 100% functions / 100% lines in coverage; lint ✓ · TSC ✓ · 1210 tests / 79 files / 100% pass · build ✓
...
- [x] **Static pages audit & update** — Privacy Policy: added portfolio/prep hub/salary data categories, updated sub-processors (Upstash Redis, Cloudmersive added; Stripe "(future)" → active; Groq model names correct), expanded security measures list, corrected GDPR export description; Terms: §3 full feature list, §7a Portfolio/username rules, §7b Stripe billing terms, ATS multi-provider; Cookie Policy: added missing `jobnest_theme` localStorage entry; Contact: FAQ answers with concrete specifics, `aria-invalid` changed from conditional string to boolean `!!errors.x`
- [x] **Mobile doc preview broken** — `DocPreviewDialog` switched to `h-[90dvh]` + flex-fill iframe (`flex-1 min-h-0`) so the PDF fills the dialog correctly on all screen sizes
- [x] **Filter bar redesign (all screens)** — horizontal pill row removed entirely; universal Filter dropdown with active-count badge (1/2) and removable active-filter chips below the search row; "Clear all" link when 2+ filters active; `db-filter-bar` CSS simplified to always `flex-direction: column`
- [x] **Notes field auto-grow + max cap** — `<Textarea autoGrow>` prop added; component auto-resizes with content; cap set to `.max(50000)` (was 5 000; removed cap was a security hole — users could POST multi-MB payloads); `useEffect` dep array fixed to `[resize]` (prevents layout reflow on every parent render)
- [x] **CSP inline script + hydration mismatch** — `app/layout.tsx` made async; `x-nonce` from `next/headers` passed to anti-flash `<script nonce>` attribute; `suppressHydrationWarning` added (per-request nonce is intentionally different between server render and client reconciliation)
- [x] **Dashboard analytics expansion** — Row A: Monthly Breakdown (grouped bars, 6 months, Applied/Rejected/Offers), Weekday Activity (Mon–Sun with peak-day callout), Top Companies (ranked horizontal bars); Row B: Stage Funnel, Avg Salary by Source, Source Effectiveness; Application Velocity gains 4w/8w/12w period picker; analytics SELECT narrowed from `*` to only needed columns (huge perf win for users with large notes/JD fields)
- [x] **Security fixes** — (1) `notes` `.max(50000)` restored after accidental removal; (2) `groqResponse.body` null-guard → clean 502 before `pipeTo`; (3) `controller.enqueue()` wrapped in try/catch for mid-stream client disconnects; (4) JSON-LD `</script>` escape via `.replace(/<\/script>/gi, "<\\/script>")`; (5) `parseSalary` thousands-comma bug fixed (`"$90,000"` → 90, not 90000); (6) weekday chart fixed from UTC parsing (`new Date("2026-05-18")`) to local-time parsing (`new Date(y,m-1,d)`) — prevents off-by-one for UTC-5..UTC-8 users; (7) interviews analytics query now joins `job_applications(company,position)` so "Next interview" footer shows company name; (8) reminders `select("*")` narrowed to 5 fields
- [x] **Tests** — 1085 tests / 74 files / 100% pass; new: notes max constraint (5), analytics new fields × 26 (sourceEffectiveness, avgSalaryBySource, stageFunnel, weekdayActivity + timezone regression), NESTAi streaming × 4 (body-null 502, multi-chunk TransformStream, malformed SSE skip, rate-limit headers), ux-audit updated for dropdown filter bar

---

## ✅ Recently Shipped (May 2026)

- [x] **Device-local timestamps, secure URLs, autocomplete off, source brand colours** — `lib/utils/date.ts` shared formatting utility reads locale + IANA timezone from `Intl.DateTimeFormat().resolvedOptions()` so every date/time site-wide renders in the user's own locale and clock; rolled out across 20+ components (interviews, dashboard, reminders, documents, prep hub, portfolio, profile, activity timeline, kanban, etc.); `secureUrlField` Zod transformer added to `lib/validations/application.ts` — strips null bytes, rejects `javascript:/data:/vbscript:/file:/blob:` schemes, validates with `new URL()`, 2083-char DB safety floor; `autocomplete="off"` set on the base `<Input>` component so it applies globally without per-field changes (password fields retain `autocomplete="new-password"` / `autocomplete="current-password"`); `SOURCE_COLORS` record added to `config/constants.ts` with official brand colours + dark-mode variants for all 11 application sources (LinkedIn `#0A66C2`, Indeed `#003A9B`, Glassdoor `#0CAA41`, Handshake `#E8552A`, Wellfound `#111111`, Dice `#EB1C26`, Referral violet, Recruiter Outreach amber, Job Fair cyan, Company Website slate, Other muted); applied as tinted pill badges on application cards

- [x] **Developer Portfolio & Public Profile Page** — GitHub OAuth (PKCE signed state, timing-safe compare, `Buffer.from` with `"hex"` encoding fixed); `github_connections` + `github_repos` + `projects` + `application_projects` + `usernames` tables (migration 31); daily `github-sync` cron (04:00 UTC); up to 6 pinned repos for portfolio display; manual sync (rate-limited 5/hr); `ProjectsSection` (CRUD, reorder with atomic parallel PATCHes + rollback, featured flag); `LinkedInSection` (URL + 8-item self-assessed strength checklist, dot-slug regex fixed); `PortfolioSettings` (live username availability, public/private toggle, `show_email` opt-in — defaults off); public `/p/{username}` page (SSR, force-dynamic, OG tags, sections: hero, featured projects, pinned repos, skills by category, education, certifications, email CTA); three security fixes: `STATE_SECRET` prod guard, cron `CRON_SECRET` falsy bypass, user email exposed without consent; 52 new flow tests (983 total, 70 files, all green); lint ✓, TSC ✓, build 83 routes ✓
...
- [x] **Disable browser autocomplete / cached suggestions on all form fields** — `autocomplete="off"` set on the base `<Input>` component so it applies globally; password fields retain `autocomplete="new-password"` (signup/change) and `autocomplete="current-password"` (login)
- [x] **Application source — brand colours per platform** — `SOURCE_COLORS` record in `config/constants.ts`; official brand colours for all 11 sources with dark-mode variants (LinkedIn `#0A66C2`, Indeed `#003A9B`, Glassdoor `#0CAA41`, Handshake `#E8552A`, Wellfound `#111111`, Dice `#EB1C26`, Referral violet, Recruiter Outreach amber, Job Fair cyan, Company Website slate); applied as tinted pill badges on application cards
...
- [x] **GitHub integration** — uses Supabase-configured GitHub OAuth (same app as login — no separate `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET` in app env needed); connect button is client-side `signInWithOAuth`; callback exchanges PKCE code via Supabase and reads `session.provider_token`; upserts `github_connections` (**token encrypted at rest with AES-256-GCM** keyed from `CSRF_SECRET`) and `github_repos` (up to 100 repos); profile card (avatar, bio, stats, repos); pin up to 6 repos for portfolio display; manual sync (5/hr rate limit); daily cron `github-sync` at 04:00 UTC; disconnect purges connection + cached repos; `redirectTo` URL must be in Supabase → Auth → URL Configuration → Redirect URLs
- [x] **Project showcase** — `projects` table: `title`, `description`, `tags TEXT[]`, `demo_url`, `repo_url`, `image_url`, `github_repo_id` (optional link to cached repo), `is_featured`, `display_order`; full CRUD in profile; up/down reorder with atomic parallel PATCHes and rollback on partial failure; `application_projects` junction links projects to job applications
...
- [x] **LinkedIn profile sync** — LinkedIn URL stored in `user_metadata`; client-side regex validates `linkedin.com/in/…` format (dots in slug now accepted); 8-item self-assessed "LinkedIn Strength" checklist (photo, headline, about, experience, skills, featured, recommendations, 500+ connections) with score 0–8 and progress bar; saved via `POST /api/portfolio/linkedin`
- [x] **Portfolio public page** — opt-in shareable `/p/{username}` page; `usernames` table for O(1) slug lookup; SSR, `force-dynamic`, full OG/Twitter metadata; gated on `portfolio_public` flag in user_metadata; sections: hero (avatar/bio/links/GitHub stats), featured projects, pinned GitHub repos (language dots + star/fork counts + topics), other projects, skills by category, education, certifications, email CTA (opt-in via `show_email` flag — defaults off); no job application data exposed
...
- _**Security** — `appUrl` in OAuth callback/connect routes pinned to `NEXT_PUBLIC_APP_URL` (no more `x-forwarded-host` derivation for redirect targets); `image_url` upgraded from bare string to `z.string().url()` in both project schemas._
- _**Tests** — 1032 tests / 73 files all green; new: `tokens.test.ts` (14 tests: roundtrip, random IV, legacy passthrough, tamper detection), `linkedin-verify.test.ts` (12 tests: auth guard, URL regex, all fetch status branches), `date.test.ts` (11 tests: `formatCompactDateTime` current/past year, regressions), `portfolio-flow.test.ts` extended (+USERNAME DELETE, +30-day cooldown, +image_url validation, +decryptToken path); TSC ✓ · Lint ✓ · Build ✓ · Coverage 63.6%/55.2%/62.9%/66.6% (above 47/40/42/50 thresholds)._
