...
_Last updated: 11th May 2026 — Shipped: Developer Portfolio & Identity — GitHub OAuth (PKCE state, timing-safe compare, `hex` encoding fix), github_connections/github_repos tables, 6-repo pin, daily sync cron, project showcase (CRUD, reorder, application_projects junction), LinkedIn strength checklist (8-item, dot-slug fix), username claim with uniqueness table, public /p/{username} page (SSR, OG tags, show_email opt-in), 3 security fixes (STATE_SECRET prod guard, cron falsy bypass, email disclosure); migration 31; 52 new portfolio flow tests; 983 tests / 70 files all green; TSC ✓ · Lint ✓ · Build 83 routes ✓._

_Last updated: 12th May 2026 — Shipped: All 4 URGENT items — device-local timestamps (`lib/utils/date.ts`, `Intl` locale + timezone, 20+ components), secure URL validation (`secureUrlField` Zod transformer, null-byte strip, scheme blocklist, `new URL()` check), global autocomplete-off (base `<Input>` component), source brand colours (`SOURCE_COLORS` in constants, all 11 sources with dark-mode variants). Also: `@types/diff` 7→8, TruffleHog 3.95.2→3.95.3, 8 npm dep bumps (Supabase, Tailwind 4.3, Playwright 1.60, Vitest 4.1.6, @types/node 25.7)._

_Last updated: 13th May 2026 — Portfolio hardening sprint:_
- _**GitHub OAuth rewritten** to use Supabase-configured OAuth (same app as login — no separate `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET` in app env needed; callback URL stays at Supabase's URL); connect button is client-side `signInWithOAuth`; `session.provider_token` extracted in callback via PKCE exchange; GitHub tokens **encrypted at rest** with AES-256-GCM keyed from `CSRF_SECRET` (`lib/security/tokens.ts`); legacy plaintext tokens handled transparently._
- _**LinkedIn section** — URL field normalises on change + blur (bare username, missing https, /in/ prefix); new `GET /api/portfolio/linkedin/verify` endpoint does best-effort HEAD request; checklist auto-saves per-toggle with 600 ms debounce + "Self-assessed" callout added._
- _**Username management** — 30-day change cooldown enforced server-side (POST returns 409 with next-available date); new `DELETE /api/portfolio/username` removes portfolio URL, resets `portfolio_public`, two-step confirm in UI; `username_changed_at` stored in metadata._
- _**Projects** — `image_url` field added to create/edit form with live preview thumbnail; URL validation (`z.string().url()`) enforced at API level on both POST and PATCH schemas._
- _**Portfolio share URL** — shown immediately after username saved (not gated on `portfolio_public`); includes copy button + private-portfolio hint when visibility is off._
- _**Application cards** — new `Created` / `Updated` timestamp row using `formatCompactDateTime` ("May 12 at 3:45 PM" / "May 12, 2025 at 3:45 PM")._
- _**Profile page** — restructured into four visually separated groups in both left nav and right content: **Profile** (Display Name · About You · NESTAi Context) / **Career** (Work Authorization) / **Preferences** (Notifications) / **Security** (Password · Danger Zone); sidebar auth method shows exact OAuth providers (Google, GitHub) + correct password status._
