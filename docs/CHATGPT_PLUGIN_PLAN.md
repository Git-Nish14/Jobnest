...
7. Tailor a resume and, after actually applying, type JOBNEST. The keyword confirms the application, defaults status to Applied and the date to today, extracts every supported detail, always attaches a job description, and calls `save_job_application`. It asks only if company or title cannot be recovered.
...
- Save fields: required `request_id`, `company`, `position`, and `job_description`; `applied_date` defaults to today and `status` to Applied; optional `job_id`, `job_url`, `location`, `salary_range`, `notes`, `source`, `ats_provider`, `requires_sponsorship`, `company_tier`, and `glassdoor_rating`. The complete job description available in the chat is used up to 20,000 characters. When original posting text is unavailable, ChatGPT creates a detailed description from known facts and labels it `Generated from conversation:`. Unknown optional values are omitted and never invented.
...
Complete metadata extraction and migration 53 were committed and pushed to `main` as `a4152e9`. The current local, uncommitted follow-up responds to another live test where ChatGPT still omitted the description: `job_description` is now mandatory for the plugin, JOBNEST itself confirms the application, status defaults to Applied, and the application date defaults to today without asking the user. MCP instructions now include a full JSON sample, field limits, enum rules, and a factual generated-description fallback when the original posting text is unavailable. The Settings steps also provide a copyable `@Jobnest` instruction that folder/project users place above all other instructions. Targeted checks currently pass: 95 Vitest tests, TypeScript, and targeted ESLint. The 13 isolated PostgreSQL checks passed for migration 53 in the preceding committed change.
...
| Mandatory description and JOBNEST defaults | Local and uncommitted; 95 targeted Vitest tests, TypeScript, and targeted ESLint pass. Description is required, JOBNEST uses Applied/today without confirmation questions, and Settings supplies a copyable folder/project instruction. |
