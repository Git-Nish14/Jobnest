...
If Jobnest is not working in ChatGPT, the in-app setup guide tells users to open ChatGPT Settings → Plugins → Jobnest, choose reconnect when available or remove and re-add the connection using the displayed MCP URL, sign in and approve **Check and save job applications**, then permit both required actions.
