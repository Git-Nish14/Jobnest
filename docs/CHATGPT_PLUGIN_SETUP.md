...
The in-app setup guide includes a highlighted reconnect notice for existing users. It tells them to open ChatGPT Settings → Plugins → Jobnest, choose reconnect when available or remove and re-add the old connection using the displayed MCP URL, sign in and approve **Check and save job applications**, then permit both required actions.
